...
def fill_trainval_infos(data_path, nusc, train_scenes, val_scenes, test=False, max_sweeps=10, with_cam=False):
    ...
    chans = ['RADAR_LEFT_FRONT', 'RADAR_RIGHT_FRONT']  # The reference channel of the current sample_rec that the point clouds are mapped to.
    ...
    for index, sample in enumerate(nusc.sample):
        ...
        for chan in chans:
            if chan not in sample['data']:
                print(f"Warning: {chan} not in sample_data for sample {sample['token']}, skip this channel.")
                continue

            radar_token = sample['data'][chan]
            curr_sd_rec = nusc.get('sample_data', radar_token)
            current_cs_rec = nusc.get('calibrated_sensor', curr_sd_rec['calibrated_sensor_token'])
            current_pose_rec = nusc.get('ego_pose', curr_sd_rec['ego_pose_token'])
            ref_time = 1e-6 * curr_sd_rec['timestamp']

            # Transform from Sensor to Ego Car
            car_from_radar = transform_matrix(
                current_cs_rec['translation'], Quaternion(current_cs_rec['rotation']), inverse=False
            )
            
            # Transform from Ego Car to Global Map
            global_from_radar_car = transform_matrix(
                current_pose_rec['translation'], Quaternion(current_pose_rec['rotation']), inverse=False
            )

            radar_from_car = transform_matrix(
                current_cs_rec['translation'], Quaternion(current_cs_rec['rotation']), inverse=True
            )
            car_from_global_radar = transform_matrix(
                current_pose_rec['translation'], Quaternion(current_pose_rec['rotation']), inverse=True
            )
            ...
            while len(sweeps) < max_sweeps - 1:
                if curr_sd_rec['prev'] == '':
                    ...
                    # This maps: Past Radar -> Past Ego -> Global -> Present Ego -> Present Radar
                    tm = reduce(np.dot, [
                        radar_from_car,          # Present Ego -> Present Radar
                        car_from_global_radar,   # Global -> Present Ego
                        global_from_car,         # Past Ego -> Global
                        car_from_current         # Past Radar -> Past Ego
                    ])
            ...
            info['radars'][chan] = {
                'lidar_path': Path(nusc.get_sample_data_path(radar_token)).relative_to(data_path).__str__(),
                'car_from_radar': car_from_radar,
                'global_from_radar_car': global_from_radar_car,
                'sweeps': sweeps  
            }

            assert len(info['radars'][chan]['sweeps']) == max_sweeps - 1, \
                f"sweep {curr_sd_rec['token']} only has {len(info['radars'][chan]['sweeps'])} sweeps, " \
        ...
        if not test:
            ...
            # New boxes in global frame
            box = [nusc.get_box(token) for token in sample['anns']]

            # pull it down to the flat Ego/IMU frame
            for b in box:
                b.translate(-np.array(ref_pose_rec['translation']))
                b.rotate(Quaternion(ref_pose_rec['rotation']).inverse)

            locs = np.array([b.center for b in box]).reshape(-1, 3)
            dims = np.array([b.wlh for b in box]).reshape(-1, 3)[:, [1, 0, 2]]  # wlh == > dxdydz (lwh)
            velocity = np.array([b.velocity for b in box]).reshape(-1, 3)
            rots = np.array([quaternion_yaw(b.orientation) for b in box]).reshape(-1, 1)
            names = np.array([b.name for b in box])
            tokens = np.array([b.token for b in box])
